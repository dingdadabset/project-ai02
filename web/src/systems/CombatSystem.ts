...
export class CombatSystem {
    ...
    private _contactTimer: number = 0;
    private _contactInterval: number = 500;
        ...
        for (let dx = -gridR; dx <= gridR; dx++) {
            for (let dy = -gridR; dy <= gridR; dy++) {
                ...
                for (const e of cell) {
                    ...
                    const ddx = e.pos.x - x;
                    const ddy = e.pos.y - y;
                    if (ddx * ddx + ddy * ddy <= r2) result.push(e);
                }
            }
        }
        return result;
    ...
    public dealDamageToEnemy(enemy: Enemy, baseDamage: number): void {
        if (!enemy.alive) return;
        const { damage, isCrit } = this.calcPlayerDamage(baseDamage);
        enemy.takeDamage(damage, isCrit);
    }

    public dealAoeDamage(cx: number, cy: number, radius: number, baseDamage: number): void {
    ...
    private _checkContactDamage(dt: number): void {
        ...
        if (this._contactTimer < this._contactInterval) return;
    ...
    public get enemyCount(): number {
        let count = 0;
        for (const e of this.enemies) if (e.alive) count++;
        return count;
    }
