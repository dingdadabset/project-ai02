...
import { WeaponBase, createWeapon } from '../weapons/Weapon.js';
...
export class GameScene extends Scene {
    ...
    private _shockwaves: { x: number; y: number; r: number; max: number; alpha: number }[] = [];
    ...
    public update(dt: number): void {
        ...
        for (const e of this.combat.enemies) {
            if (e.alive) e.update(dt, this.player);
        }
    ...
    private _renderWorld(): void {
        ...
        // 武器特效（底层）
        for (const w of this.weapons) w.render(r);

        // 敌人
        for (const e of this.combat.enemies) {
            if (e.alive && r.isInView(e.pos.x, e.pos.y, 60)) e.render(r);
        }
        ...
        for (const p of this.combat.projectiles) {
            if (p.alive && r.isInView(p.pos.x, p.pos.y, 30)) p.render(r);
        }
    ...
    private _renderHUD(): void {
        ...
        r.drawText(`HP ${Math.ceil(this.player.stats.currentHp)} / ${this.player.stats.maxHp}`,
            padX + barW / 2, 31, { size: 13, color: '#fff', align: 'center', baseline: 'middle', bold: true,
        ...
        r.drawText(`Lv.${this.player.level}`, padX + 4, 50 - 16,
            { size: 16, color: '#7afaff', bold: true, stroke: '#000', strokeWidth: 3 });
    ...
    private _renderDeathOverlay(): void {
        const r = this.renderer;
        ...
        r.drawText(`存活 ${Math.floor(this.elapsed / 1000)}s    击杀 ${this.killCount}`,
            r.width / 2, r.height / 2 + 40, {
        ...
        if (!this.input.pointerDown && (this as any)._wasClickedAfterDeath) {
            this.game.switchTo(SCENE_KEYS.MENU);
        }
        if (this.input.pointerDown) {
            (this as any)._wasClickedAfterDeath = true;
        }
    }
    ...
    private _updateShockwaves(dt: number): void {
        const dtSec = dt / 1000;
    ...
    private _onDamage(data: { x: number; y: number; damage: number; isCrit: boolean }): void {
    ...
    private _onPlayerDmg(damage: number): void {
        this._damageNumbers.push({
            x: this.player.pos.x,
    ...
    private _onPlayerHeal(amount: number): void {
        this._damageNumbers.push({
            x: this.player.pos.x,
            y: this.player.pos.y - 20,
            text: `+${amount}`,
    ...
    private _onKill(): void {
        this.killCount++;
    }

    private _onSkillSelect(options: any[]): void {
        this.paused = true;
        this.game.switchTo(SCENE_KEYS.SKILL_SELECT, { options, gameScene: this });
    }

    private _onSkillChosen(): void {
        this.paused = false;
    }

    private _onWeaponAcquired(weaponId: string): void {
        if (this.weapons.find(w => w.id === weaponId)) return;
        if (this.weapons.length >= Balance.MAX_WEAPONS) return;
        const w = createWeapon(weaponId, this.player, this.combat, this.spawn);
        if (w) this.weapons.push(w);
    }

    private _onWeaponUpgrade(weaponId: string): void {
        const w = this.weapons.find(w => w.id === weaponId);
        if (w) w.upgrade();
    }

    private _onShockwave(data: { x: number; y: number; radius: number }): void {
