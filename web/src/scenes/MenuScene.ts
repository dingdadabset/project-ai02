...
import { ConfigManager, PlayerConfig } from '../core/ConfigManager.js';

export class MenuScene extends Scene {
    private _selectedCharIdx: number = 0;
    private _allChars: PlayerConfig[] = [];

    private _btnStart = { x: 0, y: 0, w: 320, h: 80 };
    private _btnCharLeft = { x: 0, y: 0, w: 60, h: 60 };
    private _btnCharRight = { x: 0, y: 0, w: 60, h: 60 };
    ...
    private _inRect(x: number, y: number, r: { x: number; y: number; w: number; h: number }): boolean {
    ...
    public render(): void {
        ...
        r.drawText(`HP: ${char.baseHp}    ATK: ${char.baseAtk}    SPD: ${char.baseSpeed}`,
            cx, statY, { size: 18, color: '#aaa', align: 'center' });
        ...
        r.drawRoundedRect(this._btnCharLeft.x, this._btnCharLeft.y,
            this._btnCharLeft.w, this._btnCharLeft.h, 30, '#334', '#fff');
        r.drawText('<', this._btnCharLeft.x + 30, this._btnCharLeft.y + 30,
            { size: 32, color: '#fff', align: 'center', baseline: 'middle', bold: true });

        r.drawRoundedRect(this._btnCharRight.x, this._btnCharRight.y,
            this._btnCharRight.w, this._btnCharRight.h, 30, '#334', '#fff');
        r.drawText('>', this._btnCharRight.x + 30, this._btnCharRight.y + 30,
            { size: 32, color: '#fff', align: 'center', baseline: 'middle', bold: true });

        // 开始按钮
        this._btnStart.x = cx - this._btnStart.w / 2;
        this._btnStart.y = r.height - 280;
        r.drawRoundedRect(this._btnStart.x, this._btnStart.y, this._btnStart.w, this._btnStart.h, 16,
            '#ffd24a', '#fff');
        ...
        r.drawText(`金币: ${save.gold}    钻石: ${save.diamond}    最高: ${save.maxChapter}-${save.maxStage}`,
            cx, r.height - 100, { size: 14, color: '#888', align: 'center' });
